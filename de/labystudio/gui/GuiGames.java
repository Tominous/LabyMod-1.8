  ...
  extends GuiMenuScreen
{
  ...
  public void b()
  {
    ...
    this.n.clear();
    this.n.add(this.btnPlay = new avs(0, this.l / 2 - 100, this.m - 70, 200, 20, "Play"));
    
    int bx = 0;
    for (EnumGame g : EnumGame.values()) {
      if (g != EnumGame.None)
      {
        avs b = new avs(-1, 5 + bx, this.m - 26, 80, 20, g.name());
  ...
  public void a(int mouseX, int mouseY, float partialTicks)
  {
    ...
    if (this.selectedGame == EnumGame.None) {
      this.draw.drawCenteredString(Color.cl("c") + L.f("gui_games_nogames", new Object[0]), this.l / 2, this.m / 2);
    }
    if (this.selectedGame == EnumGame.Snake)
    {
      GL11.glPushMatrix();
