...
public class UserCapesDownloader
{
  ...
  private boolean downloadUserCapes(String page)
  {
    try
    {
      ...
      connection.connect();
      Debug.debug("[UserCapes] Response: " + connection.getResponseCode());
      ...
      Debug.debug("[UserCapes] Total usercapes: " + userCapes.size());
      LabyMod.getInstance().getCapeManager().setUserCapes(userCapes);
      return true;
    }
    catch (Exception error)
    {
      Debug.debug("[UserCapes] Error: " + error.getMessage());
